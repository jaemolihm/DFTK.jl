...
@testset "prepare_band_data" begin
    ...
    @test ret.branches[2].kindices == [3, 4, 5]
    @test ret.branches[3].kindices == [6, 7, 8]
    ...
    for iband in 1:4
        ...
        @test ret.branches[2].λ[:, iband, 1] == [10ik .+ iband for ik in 3:5]
        @test ret.branches[3].λ[:, iband, 1] == [10ik .+ iband for ik in 6:8]
    ...
    for ibr in 1:3
        ...
        ibr != 2 && (ikpt += 1) # branch 2 is continuous with branch 1, so ik does not change
        for ik in 2:3
            ref_kdist[ik, ibr] = (
